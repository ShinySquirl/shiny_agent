...
def analyze_email_with_gpt4(email_body):
    client = OpenAI(api_key=API_KEY)
    response = client.Completion.create(
        engine="text-davinci-002",  # Use the appropriate GPT-4 model name
        prompt=email_body,
        max_tokens=150
    )
...
def main():
    # Replace 'your_client_secrets.json' with the path to your client secrets file.
    # Ensure the scopes match your application's requirements.
    ...
    for email in emails:
        ...
        print("\n---\n")
        analysis = analyze_email_with_gpt4(email['body'])
        print(f"Analysis for email from {email['from']} to {email['to']}: {analysis}")
